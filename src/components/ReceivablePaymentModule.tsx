...
import { ReceivablePayment, Customer, SalesInvoice } from '../types';
import { Search, Plus, X, Check, Trash2, Coins, ArrowLeft, ArrowRight, Eye } from 'lucide-react';
import SearchableSelect from './SearchableSelect';

interface ReceivablePaymentModuleProps {
  authToken: string;
  showToast: (msg: string, status: 'success' | 'error') => void;
}

export default function ReceivablePaymentModule({
  authToken,
  showToast
}: ReceivablePaymentModuleProps) {
  ...
  // Filtering invoices for selected customer
  const filteredInvoices = salesInvoices.filter(
    si => si.customerId === formCustomerId && (si.status === 'Unpaid' || si.status === 'Draft')
  );
  ...
  // When a specific invoice is selected, pre-fill its outstanding balance
  const handleInvoiceChange = (invId: string) => {
    setFormInvoiceId(invId);
    const selectedInv = filteredInvoices.find(si => si.id === invId);
    if (selectedInv) {
  ...
  const handleCreatePayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formCustomerId || !formInvoiceId || !formPaymentDate || !formPaymentMethod || !formAmount) {
    ...
    try {
      ...
      showToast(`Receivable payment [${resData.paymentNumber}] for ${resData.amount.toLocaleString()} posted.`, 'success');
      setIsFormOpen(false);
      ...
      // Reload
      fetchPayments();
      fetchSalesInvoices(); // Status changed to Paid
    } catch (err: any) {
  ...
  const handleDeletePayment = async (paymentId: string, paymentNum: string) => {
    if (!confirm(`Are you sure you want to delete payment [${paymentNum}]? This will restore the Sales Invoice to "Unpaid" status.`)) return;
  ...
  const handleOpenDetail = (p: ReceivablePayment) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-950 font-sans tracking-tight">Receivable Payments</h1>
          </div>
          <p className="text-xs text-slate-500 mt-1 font-semibold">
            Track and log collection of customer invoice payments.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-100 border-b border-slate-200 text-slate-600 font-mono text-[9px] uppercase tracking-wider select-none">
                ...
                <th className="py-2.5 px-4 font-bold">Date Received</th>
                <th className="py-2.5 px-4 font-bold">Method</th>
                <th className="py-2.5 px-4 font-bold text-rightBig text-right">Settled Amount</th>
            ...
            <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-400 italic">
                    No receipt logs posted. Close of balance is healthy.
                  </td>
                </tr>
              ) : (
                filteredPayments.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3 px-4 truncate max-w-xs">{p.customerName || 'Unknown Customer'}</td>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-screen shadow-xl flex flex-col animate-slide-in">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wide">POST CUSTOMER PAYMENT RECEIPT</h2>
                ...
                <div className="space-y-1.5 animate-fade-in">
                  ...
                  {filteredInvoices.length === 0 ? (
                    <div className="p-3 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded-lg text-[11px] font-medium font-sans">
                      Done! This client holds 0 outstanding unpaid invoices. All balances settled.
                    </div>
                  ) : (
                    <select
                      value={formInvoiceId}
                    ...
                    >
                      ...
                      {filteredInvoices.map(si => (
                        <option key={si.id} value={si.id}>
                          {si.siNumber} (${si.totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}) - Due {si.dueDate}
                        </option>
                      ))}
                    </select>
                  )}
                ...
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      placeholder="Bank wire reference code, cash drawer ID, etc..."
                      rows={3}
                      className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-xs font-semibold focus:outline-none focus:border-slate-400"
                    />
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-screen shadow-xl flex flex-col animate-slide-in">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div>
                <span className="px-2 py-0.5 bg-blue-100 border border-blue-200 rounded text-[9px] uppercase tracking-wider font-mono text-blue-700 font-bold block w-fit">
                  RECEIPT LOG
                </span>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="bg-slate-50 border border-slate-100 rounded-lg p-4 space-y-3 font-mono text-[11px] text-slate-700">
                <div className="flex justify-between">
                  <span className="text-slate-400 uppercase font-sans font-bold text-[9px]">Receipt Date:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-slate-400 uppercase font-sans font-bold text-[9px]">Entity:</span>
                  <span className="text-slate-900 font-semibold truncate max-w-[200px]">{selectedPayment.customerName}</span>
                </div>
              ...
              {selectedPayment.remarks && (
                <div className="space-y-1.5">
                  <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider font-mono">Remarks / Reference log</span>
